...
Currently configured project: `agent-artifacts` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/agent-artifacts/SETUP.md` to fill in
   `data/agent-artifacts/INFO.md`.
...
`--project-id agent-artifacts` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/agent-artifacts/`.
