# Agent setup for `agent-artifacts`

This is a deepsec scanning workspace. Project `agent-artifacts` was just registered
(target: `..`). Setup is incomplete — `data/agent-artifacts/INFO.md`
...
2. **Fill in `data/agent-artifacts/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id agent-artifacts
pnpm deepsec process --project-id agent-artifacts
```
