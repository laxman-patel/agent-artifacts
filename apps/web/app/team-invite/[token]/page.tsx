...
export default async function TeamInvitePage(props: { params: Promise<{ token: string }> }) {
  const { token } = await props.params;
  const cookieStore = await cookies();
  const header = cookieHeader(cookieStore);
  const profile = await fetchProfileMe(header);
  const nextPath = `/team-invite/${encodeURIComponent(token)}`;
    ...
    <main className="page-shell narrow">
      <header className="page-header">
        <div>
          ...
          <h1>Join team</h1>
          <p className="subtle">Signed in as {profile.body.user.email}. Accept this invitation to join the team.</p>
        </div>
      </header>
      ...
      <section className="card flat stack">
        <div className="section-header">
          <h2>Invitation</h2>
          <p className="muted small">Accepting adds this account to the team immediately.</p>
        </div>
