---
title: "Access Model"
description: "Principals, roles, scopes, public access, share links, and policy enforcement."
---

Every request is evaluated as a principal against the same policy layer, whether it comes from the web app, REST API, CLI, or MCP server.

## Principals

Artifacts recognizes these principal types:
...
Principals may also carry scopes and artifact role grants.
...
Roles control artifact actions:
...
Private artifacts require an owning user, workspace member, email allowlist match, API key, OAuth principal, or share link grant.

## Email Allowlists

Access settings can include viewer email grants:
...
Email allowlists require a plan that includes private artifacts and email allowlists.

## Share Links

Share links grant `viewer` or `editor` access to a single artifact. Links can optionally expire.
...
API keys are bearer credentials with explicit scopes:
...
<Warning>
  Treat API keys as secrets. The raw key is only useful at creation time; store it in a secret manager and pass it through environment variables or stdin.
</Warning>
