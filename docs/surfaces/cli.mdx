...
Upload `apps/cli/dist/release/v<version>/` and `apps/cli/dist/release/latest/` to the R2-backed download domain at `https://downloads.hostartifacts.dev/cli/`. The public web app on Railway exposes `https://hostartifacts.dev/install.sh` as the memorable installer URL.
