...
The public installer requires Node.js 24+, downloads the Node-based CLI, installs it to `~/.local/bin/artifacts`, and then delegates skill installation to Vercel's `skills` CLI. The skill can be installed for Cursor, Claude Code, Codex, OpenCode, Copilot, and other supported agents.
